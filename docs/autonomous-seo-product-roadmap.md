...
| P8 | **Release freshness and indexation** — repeated live checks after P3–P6 served old prerendered pages | Ensure shipped product and SEO changes reach users and crawlers | GitHub `main` advances and local Cloudflare-compatible builds pass, but no Actions/status records exist and the public Worker has repeatedly returned stale page titles | Diagnose repository deployment wiring and cache/version behavior without changing Cloudflare settings, secrets, DNS, domains or infrastructure | Verify canonical pages, sitemap freshness and response headers after a confirmed deployment | Planned | Pending | — |
...
- **Next recommended increment:** Investigate release freshness and indexation (P8) because repeated public checks have served old prerendered pages despite validated `main` commits. Make no Cloudflare infrastructure change without approval.
